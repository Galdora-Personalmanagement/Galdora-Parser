...
# Alle notwendigen Pfade hinzufügen für Streamlit Cloud
paths_to_add = [
    project_root,
...
# Importe aus den reorganisierten Modulen - Streamlit Cloud kompatibel
try:
    ...
    from ui.utils.ui_helpers import get_logo_as_base64
except ImportError:
    ...
    from src.ui.utils.ui_helpers import get_logo_as_base64
    ...
    if mode_selection == "📄 Lebenslauf hochladen":
        ...
        with col2:
            if st.button("🚀 Weiter zu Schritt 2 - Profil bearbeiten", use_container_width=True, type="primary"):
                ...
                st.session_state.empty_profile_data = {
                    "persönliche_daten": {
                        "name": "",
                        "wohnort": "",
                        "jahrgang": "",
                ...
                st.session_state.profile_data = st.session_state.empty_profile_data
                st.session_state.step = 2  # Direkt zu Schritt 2 springen
...
# Schritt 2: Profil bearbeiten (für leere Vorlagen oder separate Navigation)
elif st.session_state.step == 2:
    st.subheader("2. Profil erstellen und exportieren")
    
    # Prüfe ob wir Profildaten haben (aus Upload oder leer)
    if 'profile_data' not in st.session_state:
        st.error("Keine Profildaten gefunden. Bitte kehren Sie zu Schritt 1 zurück.")
    ...
    with tab1:
        ...
        with col1:
            ...
            # Aktuellen Führerscheineintrag verarbeiten
            current_fuehrerschein = personal_data.get("führerschein", "")
            default_selected = []
        ...
        edited_data["verfuegbarkeit_status"] = verfuegbarkeit_status
        edited_data["verfuegbarkeit_details"] = ""
        ...
        # KI-Aufgaben Funktionen definieren
        def generate_ai_tasks_for_position(position, firma):
            """Generiert 4 passende Aufgaben für eine Position mit KI"""
            try:
                # OpenAI API Key aus der Session holen
                api_key = st.session_state.get('openai_api_key')
                if not api_key:
                    return "• Keine OpenAI API Key verfügbar\n• Bitte API Key in der Seitenleiste eingeben\n• Dann erneut versuchen\n• KI-Aufgaben können nicht generiert werden"
                
                from openai import OpenAI
                client = OpenAI(api_key=api_key)
                
                prompt = f"""
                Erstelle genau 4 präzise, professionelle Aufgaben/Tätigkeiten für folgende Position:
                ...
                Anforderungen:
                - Genau 4 Stichpunkte
                - Jeder Stichpunkt beginnt mit "• "
                - Realistische, branchenspezifische Aufgaben
                ...
                response = client.chat.completions.create(
                    model="gpt-3.5-turbo",
                    messages=[
                        {"role": "system", "content": "Du bist ein Experte für Personalwesen und Stellenbeschreibungen. Erstelle präzise, professionelle Aufgabenbeschreibungen."},
                ...
                return f"• Fehler bei der KI-Generierung: {str(e)}\n• Bitte API Key prüfen\n• Internetverbindung prüfen\n• Erneut versuchen"
        
        # Einfache Form für manuelle Eingabe von Berufserfahrung, Ausbildung etc.
        # Berufserfahrung hinzufügen
        st.markdown("#### Berufserfahrung")
        
        # KI-Buttons für alle Berufserfahrungen
        col1, col2, col3 = st.columns([2, 2, 4])
        with col1:
            if st.button("🤖 KI-Aufgaben für alle generieren", help="Generiert automatisch 4 passende Aufgaben für jede Berufserfahrung"):
                berufserfahrung_temp = profile_data.get("berufserfahrung", [])
                if st.session_state.get("neue_berufserfahrung"):
                    berufserfahrung_temp.extend(st.session_state.neue_berufserfahrung)
                
                with st.spinner("KI generiert Aufgaben..."):
                    for i, exp in enumerate(berufserfahrung_temp):
                        position = exp.get("position", "")
                        firma = exp.get("firma", "")
                        
                        if position and firma:  # Nur generieren wenn Position und Firma vorhanden
                            ki_aufgaben = generate_ai_tasks_for_position(position, firma)
                            exp["beschreibung"] = ki_aufgaben
                            # Session State Key für Text Area aktualisieren
                            st.session_state[f"be_beschreibung_{i}"] = ki_aufgaben
                    
                    st.success("KI-Aufgaben für alle Berufserfahrungen generiert!")
                    st.rerun()
        
        with col2:
            if st.button("🗑️ Alle KI-Aufgaben entfernen", help="Entfernt alle automatisch generierten Aufgaben"):
                berufserfahrung_temp = profile_data.get("berufserfahrung", [])
                if st.session_state.get("neue_berufserfahrung"):
                    berufserfahrung_temp.extend(st.session_state.neue_berufserfahrung)
                ...
                for i, exp in enumerate(berufserfahrung_temp):
                    exp["beschreibung"] = ""
                    # Session State Key für Text Area leeren
                    st.session_state[f"be_beschreibung_{i}"] = ""
                
                st.success("Alle KI-Aufgaben entfernt!")
                st.rerun()
        
        if st.button("➕ Neue Berufserfahrung hinzufügen"):
            if "neue_berufserfahrung" not in st.session_state:
                st.session_state.neue_berufserfahrung = []
            st.session_state.neue_berufserfahrung.append({
                "zeitraum": "",
                "firma": "", 
                "position": "",
                "beschreibung": ""
        ...
        # Zeige bestehende Berufserfahrung
        berufserfahrung = profile_data.get("berufserfahrung", [])
        if st.session_state.get("neue_berufserfahrung"):
            berufserfahrung.extend(st.session_state.neue_berufserfahrung)
        
        for i, exp in enumerate(berufserfahrung):
            st.markdown(f"**Berufserfahrung {i+1}**")
            col1, col2 = st.columns(2)
            with col1:
                zeitraum = st.text_input(f"Zeitraum", value=exp.get("zeitraum", ""), key=f"be_zeitraum_{i}")
                firma = st.text_input(f"Firma", value=exp.get("firma", ""), key=f"be_firma_{i}")
            with col2:
                position = st.text_input(f"Position", value=exp.get("position", ""), key=f"be_position_{i}")
                beschreibung = st.text_area(f"Beschreibung", value=exp.get("beschreibung", ""), key=f"be_beschreibung_{i}")
        ...
        # Speichere die bearbeiteten Berufserfahrungen
        edited_data["berufserfahrung"] = berufserfahrung
        edited_data["ausbildung"] = profile_data.get("ausbildung", [])
        edited_data["weiterbildungen"] = profile_data.get("weiterbildungen", [])
        
        # Aktualisiere die Session-Daten
        updated_profile_data = {
            "persönliche_daten": edited_data,
            "berufserfahrung": edited_data["berufserfahrung"],
            "ausbildung": edited_data["ausbildung"], 
            "weiterbildungen": edited_data["weiterbildungen"]
        }
        st.session_state.profile_data = updated_profile_data
    
    with tab2:
        # Export-Tab (vereinfacht für den Start)
        st.markdown("### Profil exportieren")
        
        # Nur Classic Template verfügbar
        template_to_use = "classic"  # Einziges verfügbares Template
        st.info("📄 Klassisches Template wird verwendet")
        
        # Profil generieren und herunterladen
        if st.button("📄 Profil generieren", type="primary", use_container_width=True):
            with st.spinner("Profil wird generiert..."):
                try:
                    selected_company = st.session_state.get('selected_company', 'galdora')
                    generator = ProfileGenerator(selected_company=selected_company)
                    ...
                    # Generiere Profil
                    profile_path = generator.generate_profile(updated_profile_data, output_path, template=template_to_use)
                    st.session_state.preview_pdf = profile_path
                    st.session_state.selected_template = template_to_use
                    ...
                    name = updated_profile_data.get("persönliche_daten", {}).get("name", "Unbekannt").replace(" ", "-")
                    ...
                    docx_path = docx_output.name
                    docx_output.close()
                    st.session_state.temp_files.append(docx_path)
                    
                    try:
                        docx_file = generator.generate_profile(updated_profile_data, docx_path, template=template_to_use, format="docx")
                        ...
                        st.error(f"Fehler bei Word-Export: {str(e)}")
                        
                except Exception as e:
                    st.error(f"Fehler bei der Profil-Generierung: {str(e)}")
        
        # Zeige Vorschau an, falls vorhanden
        if st.session_state.get('preview_pdf') and os.path.exists(st.session_state.preview_pdf):
            st.markdown("#### 📄 Profil-Vorschau")
            try:
                display_pdf_with_pdfjs(st.session_state.preview_pdf)
            except Exception as e:
